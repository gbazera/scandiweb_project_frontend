    ...
    return (
        <Link to={`/product/${product.id}`} className='p-4 rounded-xs hover:shadow-lg transition-shadow duration-200 relative group'>
            <div className="relative">
                ...
                <img src={product.gallery[0]} alt={product.name} className={`aspect-square w-full object-cover object-top ${!product.in_stock ? 'opacity-25' : ''}`} />
            </div>
            <h3 className={`text-lg mt-6 ${!product.in_stock ? 'text-gray-400' : ''}`}>{product.name}</h3>
            <p className={`text-lg font-semibold ${!product.in_stock ? 'text-gray-400' : ''}`}>{price.currency.symbol}{price.amount.toFixed(2)}</p>
            {product.in_stock && (
                <button
