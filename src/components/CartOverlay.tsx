                ...
                {cartItems.map((item) => (
                    <div key={item.id} className="flex gap-4 not-last:border-b not-last:pb-8 border-gray-300">
                                        ...
                                        <div className="flex gap-2 mt-2">
                                            {attribute.items.map((attrItem) => (
                                                <div
                                                    ...
                                                    `}
                                                    style={attribute.type === 'swatch' ? { backgroundColor: attrItem.value } : undefined}
                                                    data-testid={
                                                        item.selectedAttributes[attribute.name] === attrItem.value
                                                            ? `cart-item-attribute-${attribute.name.replace(/\s+/g, '-').toLowerCase()}-${attribute.name.replace(/\s+/g, '-').toLowerCase()}-selected`
                                                            : `cart-item-attribute-${attribute.name.replace(/\s+/g, '-').toLowerCase()}-${attribute.name.replace(/\s+/g, '-').toLowerCase()}`
                                                    }
                                                >
                        ...
                        <div className="flex flex-col items-center justify-between">
                            ...
                            <span className="text-lg font-semibold" data-testid='cart-item-amount'>{item.quantity}</span>
                            <button
                                onClick={() => { if(item.quantity) updateItemQuantity(item.id, item.quantity - 1) }}
                                className="w-8 h-8 border border-black text-2xl cursor-pointer"
